...
public:
  ...
  static constexpr auto k_start_delay = std::chrono::seconds(1);
  ...
  void Report(uint64_t progress_id, std::string title, std::string details,
              uint64_t completed, uint64_t total, TimePoint now);

  /// Flush any deferred events whose deadlines have passed. `now` is the
  /// wall time observed by the caller.
...
private:
  ...
  struct PendingProgress {
    ...
    std::string title;
    std::string initial_details; // The detail from the first report.
