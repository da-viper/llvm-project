...
void ProgressEventReporter::Report(uint64_t progress_id, std::string title,
  ...
  if (is_new_progress) {
    ...
    pending.initial_details = details;
...
void ProgressEventReporter::Drain(TimePoint now) {
  std::lock_guard<std::mutex> lock(m_mutex);
...
void ProgressEventReporter::Flush(uint64_t progress_id,
  ...
  auto percentage = [&]() -> std::optional<uint32_t> {
    const bool is_deterministic = pending.latest_total != UINT64_MAX;

    if (!is_deterministic || pending.latest_total == 0)
      return std::nullopt;
    auto ratio = static_cast<double>(pending.latest_completed) /
                 static_cast<double>(pending.latest_total);
    return uint32_t(ratio * 100.0);
  };
  auto send_start = [&] {
    ProgressStartEventBody body;
    body.progressId = std::to_string(progress_id);
    body.title = pending.title;
    body.message = pending.initial_details;
