...
void DAP::ProgressEventThread(lldb::SBListener listener) {
  ...
  while (!done) {
    // Bounded wait so we get a periodic tick to flush deferred progress events.
    constexpr uint32_t poll_time = ProgressEventReporter::k_start_delay.count();
    if (!listener.WaitForEvent(poll_time, event)) {
      m_progress_reporter.Drain(steady_clock::now());
      continue;
    }
    ...
    SBStructuredData data = SBDebugger::GetProgressDataFromEvent(event);
    ...
    m_progress_reporter.Report(progress_id, std::move(title),
