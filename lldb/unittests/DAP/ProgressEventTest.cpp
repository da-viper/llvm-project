...
protected:
  ProgressEventReporter::TimePoint m_now{std::chrono::seconds(0)};
  std::vector<Event> sent_events;
  ProgressEventReporter m_reporter{[this](Event e) {
    // set sequence id to prevent assertion in serialization.
    e.seq = 1;
    sent_events.push_back(std::move(e));
  }};

  void IncreaseTime(std::chrono::milliseconds d) { m_now += d; }
  ...
  void ReportAt(std::chrono::milliseconds offset, uint64_t id,
    ...
    IncreaseTime(offset);
    m_reporter.Report(id, title.str(), details.str(), completed, total, m_now);
  }
};
...
TEST_F(ProgressEventReporterTest, EmitsStartAndEndProgressAfterStartDelay) {
  ...
  ReportAt(ProgressEventReporter::k_start_delay + 500ms, 1, title,
           "done reading.", 3, 3);

  auto expected_events = ElementsAre(
      Progress("progressStart", HasTitle(title), HasMessage("in folder")),
      Progress("progressEnd", HasMessage("done reading.")));
...
TEST_F(ProgressEventReporterTest, ThrottlesUpdatesWithinInterval) {
  // Send the start.
  ReportAt(0ms, 10, "throttled", "d", 0, 10);
  ReportAt(ProgressEventReporter::k_start_delay + 10ms, 10, "throttled", "d1",
           1, 10);
  ASSERT_EQ(sent_events.size(), 1U);

  // Report an update before the update_internal.
  ReportAt(ProgressEventReporter::k_update_interval - 50ms, 10, "throttled",
           "d2", 2, 10);
  ASSERT_EQ(sent_events.size(), 1U);

  // Report an update after the update_interval.
  ReportAt(ProgressEventReporter::k_update_interval * 2, 10, "throttled", "d3",
           3, 10);

  // Report end.
  ReportAt(0ms, 10, "", "throttled end", 10, 10);

  auto expected_events = ElementsAre(
      Progress("progressStart", HasTitle("throttled")),
      Progress("progressUpdate", HasMessage("d3")),
      Progress("progressEnd", HasId(10), HasMessage("throttled end")));
  ASSERT_THAT(sent_events, expected_events);
}

TEST_F(ProgressEventReporterTest, EndFlushesLatestSnapshotEvenWhenThrottled) {
  ReportAt(0ms, 1, "t", "d0", 0, 10);
  ReportAt(ProgressEventReporter::k_start_delay + 100ms, 1, "t", "d1", 1, 10);
  ASSERT_EQ(sent_events.size(), 1U);
  ASSERT_THAT(sent_events[0], Progress("progressStart", HasTitle("t")));

  // Throttled update.
  ReportAt(50ms, 1, "", "d2", 2, 10);
  ASSERT_EQ(sent_events.size(), 1U);

  // Report end.
  ReportAt(0ms, 1, "t", "final", 10, 10);
...
TEST_F(ProgressEventReporterTest, IndependentIdsAreTrackedSeparately) {
  ReportAt(0ms, 1, "one", "d1", 0, 3);
  ReportAt(0ms, 2, "two", "d2", 0, 3);
  ReportAt(ProgressEventReporter::k_start_delay + 500ms, 1, "one", "d1", 3, 3);
  ReportAt(0ms, 2, "two", "d2", 3, 3);

  EXPECT_THAT(sent_events,
              ElementsAre(Progress("progressStart", HasId(1)),
                          Progress("progressEnd", HasId(1), HasMessage("d1")),
                          Progress("progressStart", HasId(2)),
                          Progress("progressEnd", HasId(2), HasMessage("d2"))));
}
